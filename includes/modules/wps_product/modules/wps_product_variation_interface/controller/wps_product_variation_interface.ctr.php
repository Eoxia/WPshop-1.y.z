...
class wps_product_variation_interface {
	...
	private function meta_box_variation( $post ) {
		...
		wp_localize_script( 'wps-product-variation-interface-script', 'wps_product_variation_interface',
			array(
			'variation' => $this->variations,
			'nonce_delete' => wp_create_nonce( 'wpshop_variation_management' ),
			'variation_value' => $wpdb->get_results( $query ),
			...
			'label_file' => '<span class="dashicons dashicons-upload"></span>',// __('Click to add file', 'wpshop')
		) );
	...
	private function save_post( $post_id, $post ) {
		...
		$publishs = $drafts = array();
		foreach ( $_REQUEST['wps_pdt_variations'] as $variation_id => $data ) {
			if ( isset( $data['status'] ) && 'on' === $data['status'] ) {
				array_push( $publishs, $variation_id );
			} else {
				array_push( $drafts, $variation_id );
			}
		}
		global $wpdb;
		if ( ! empty( $publishs ) ) {
			$publishs = '(' . implode( ',', $publishs ) . ')';
			$wpdb->query( $wpdb->prepare( "UPDATE $wpdb->posts SET post_status = %s WHERE ID IN $publishs", 'publish' ) );
		}
		if ( ! empty( $drafts ) ) {
			$drafts = '(' . implode( ',', $drafts ) . ')';
			$wpdb->query( $wpdb->prepare( "UPDATE $wpdb->posts SET post_status = %s WHERE ID IN $drafts", 'draft' ) );
		}
	}
	/**
